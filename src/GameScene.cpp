...
GameScene::GameScene(Game& game)
    : m_game(game), m_player(PLAYER_RADIUS), m_velocity({0.f, 0.f}), m_bgColor(sf::Color::Black) {
    ...
    m_player.setPosition({static_cast<float>(size.x) / 2.f, static_cast<float>(size.y) / 2.f});
}
...
void GameScene::update(sf::Time dt) {
    ...
    if (m_bgHue < 60.f) {
        ...
        gp = x;
        bp = 0;
    } else if (m_bgHue < 120.f) {
        rp = x;
        gp = c;
        bp = 0;
    } else if (m_bgHue < 180.f) {
        rp = 0;
        gp = c;
        bp = x;
    } else if (m_bgHue < 240.f) {
        rp = 0;
        gp = x;
        bp = c;
    } else if (m_bgHue < 300.f) {
        rp = x;
        gp = 0;
        bp = c;
    } else {
    ...
    m_bgColor = sf::Color(static_cast<std::uint8_t>((rp + m) * 255.f),
